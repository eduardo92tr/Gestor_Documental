...
document.addEventListener('DOMContentLoaded', function () {
  inputBusqueda.addEventListener('keyup', function (e) {
    console.log("Mostrando abajo input busqueda");
      ...
      if (res.length > 0) {
        res.forEach(archivo => {
          html += `<h5 class="card-title">
              <a href="${base_url + 'Assets/archivos/' + archivo.id_carpeta + '/' + archivo.nombre}" download="${archivo.nombre}">${archivo.nombre}</a></h5><hr>`;
        });
        html += `</div>
        </div>`;
      } else {
